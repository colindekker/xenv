...
SET XENV_SERVICE=tomcat
SET XENV_SERVICE_TYPE=server
SET XENV_SERVICE_VERSION=%TOMCAT_VERSION%
...
ECHO Service
  ECHO - checking for existing service instance
    ...
    IF NOT ERRORLEVEL 1 (
      ...
      ECHO   - removing
        CALL :DELETESERVICE "%XENV_SERVICE_ID%" "%XENV_SERVICE_TYPE%" "%XENV_SERVICE%" "%XENV_SERVICE_VERSION%" "%CONF_DIR%" "%ROOT_DIR%" "%DATA_DIR%" "%LOG_DIR%" "%TMP_DIR%"
        IF NOT ERRORLEVEL 1 (
          ECHO     - removed
        )
    )
...
:DELETESERVICE
  ...
  SET CATALINA_HOME=%ROOT_DIR%
  
  ECHO      - using command       "%XENV%\%XENV_SERVICE_TYPE%\%XENV_SERVICE%\%XENV_SERVICE_VERSION%\bin\service.bat" remove %XENV_SERVICE_ID%
  START /wait CMD /C "%XENV%\%XENV_SERVICE_TYPE%\%XENV_SERVICE%\%XENV_SERVICE_VERSION%\bin\service.bat" remove %XENV_SERVICE_ID%
  ::>NUL 2>&1
...
:CREATESERVICE
  ...
  ECHO     - using command     "%XENV%\%XENV_SERVICE_TYPE%\%XENV_SERVICE%\%XENV_SERVICE_VERSION%\bin\service.bat" install %XENV_SERVICE_ID%
    START /wait CMD /C %XENV%\%XENV_SERVICE_TYPE%\%XENV_SERVICE%\%XENV_SERVICE_VERSION%\bin\service.bat install %XENV_SERVICE_ID%
    ::>NUL 2>&1

    IF NOT ERRORLEVEL 1 (
      sc config %XENV_SERVICE_ID% DisplayName=%XENV_SERVICE_ID%
      ECHO     - installed
      ECHO   - start
