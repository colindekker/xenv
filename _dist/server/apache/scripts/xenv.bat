...
SET XENV_SERVICE=apache
SET XENV_SERVICE_TYPE=server
SET XENV_SERVICE_VERSION=%APACHE_VERSION%
...
:DELETESERVICE
  ...
  ECHO      - using command sc delete "%XENV_SERVICE_ID%"  
 "%XENV%\%XENV_SERVICE_TYPE%\%XENV_SERVICE%\%XENV_SERVICE_VERSION%\bin\httpd.exe" -k uninstall -n "%XENV_SERVICE_ID%">NUL 2>&1
  IF NOT ERRORLEVEL 1 (
    ECHO     - removed
  )
...
:CREATESERVICE
  ...
  ECHO     - using command "%XENV%\%XENV_SERVICE_TYPE%\%XENV_SERVICE%\%XENV_SERVICE_VERSION%\bin\httpd.exe" -k install -n %XENV_SERVICE_ID% -f "%XENV%\%XENV_SERVICE_TYPE%\%XENV_SERVICE%\_xenv\conf\httpd.conf"
    "%XENV%\%XENV_SERVICE_TYPE%\%XENV_SERVICE%\%XENV_SERVICE_VERSION%\bin\httpd.exe" -k install -n %XENV_SERVICE_ID% -f "%XENV%\%XENV_SERVICE_TYPE%\%XENV_SERVICE%\_xenv\conf\httpd.conf"
    ::>NUL 2>&1
    IF NOT ERRORLEVEL 1 (
      ECHO     - installed
      ECHO   - start
